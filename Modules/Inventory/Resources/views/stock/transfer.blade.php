...
<style>
    ...
    .page-header h1 svg { width: 28px; height: 28px; color: #7c3aed; }
    ...
    .form-section-title { font-size: 13px; font-weight: 600; color: #7c3aed; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 16px; }
    ...
    .form-control:focus { outline: none; border-color: #7c3aed; box-shadow: 0 0 0 3px rgba(124, 58, 237, 0.15); }
    ...
    .ts-wrapper.focus .ts-control { border-color: #7c3aed !important; box-shadow: 0 0 0 3px rgba(124, 58, 237, 0.15) !important; }
    ...
    .info-icon { width: 44px; height: 44px; border-radius: 8px; background: #7c3aed; display: flex; align-items: center; justify-content: center; color: #fff; font-weight: 700; font-size: 16px; }
    ...
    .info-badge { display: inline-block; padding: 4px 10px; border-radius: 12px; font-size: 11px; font-weight: 600; margin-top: 6px; background: #ede9fe; color: #5b21b6; }

    .location-box { border-radius: 10px; padding: 16px; margin-bottom: 16px; }
    .location-box.from { background: #fef2f2; border: 1px solid #fecaca; }
    .location-box.to { background: #ecfdf5; border: 1px solid #a7f3d0; }
    .location-box-title { font-size: 13px; font-weight: 700; margin-bottom: 12px; color: #111; }
    .transfer-arrow { text-align: center; padding: 8px 0; font-size: 24px; color: #7c3aed; }
    ...
    .stock-value { font-size: 20px; font-weight: 700; color: #7c3aed; }
    
    .unit-hint { background: #ede9fe; border: 1px solid #c4b5fd; border-radius: 6px; padding: 10px 12px; margin-top: 8px; font-size: 12px; color: #5b21b6; display: none; }
    ...
    .warning-box { background: #fef3c7; border: 1px solid #fbbf24; border-radius: 8px; padding: 12px 16px; margin-bottom: 16px; color: #92400e; font-weight: 500; display: none; }
...
<div class="page-container">
    <div class="page-header">
        ...
        <h1><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg> Transfer Stock</h1>
    </div>
    ...
    <div class="form-card">
        <div class="form-card-header"><h3 class="form-card-title">🔄 Transfer Stock Between Locations</h3></div>
        <div class="form-card-body">
            <form action="{{ route('inventory.stock.transfer.store') }}" method="POST" id="mainForm">
                ...
                <div class="location-box from">
                    ...
                    <div class="form-row">
                        <div class="form-group"><label class="form-label">Warehouse <span class="required">*</span></label><select name="from_warehouse_id" id="from_warehouse_id" required><option value="">Select warehouse...</option>@foreach($warehouses as $wh)<option value="{{ $wh->id }}" {{ $wh->is_default ? 'selected' : '' }}>{{ $wh->name }}</option>@endforeach</select></div>
                        <div class="form-group"><label class="form-label">Rack</label><select name="from_rack_id" id="from_rack_id"><option value="">Select rack...</option></select></div>
                    </div>
                </div>
                ...
                <div class="location-box to">
                    ...
                    <div class="form-row">
                        <div class="form-group"><label class="form-label">Warehouse <span class="required">*</span></label><select name="to_warehouse_id" id="to_warehouse_id" required><option value="">Select warehouse...</option>@foreach($warehouses as $wh)<option value="{{ $wh->id }}">{{ $wh->name }}</option>@endforeach</select></div>
                        <div class="form-group"><label class="form-label">Rack</label><select name="to_rack_id" id="to_rack_id"><option value="">Select rack...</option></select></div>
                    </div>
                </div>

                <div class="warning-box" id="sameLocationWarning">⚠️ Source and destination cannot be the same!</div>

                <div class="stock-display" id="stockDisplay"><div><div class="stock-label">Available at Source</div><div class="stock-value" id="availStock">0 PCS</div></div></div>
                ...
                <div class="form-section">
                    <div class="form-group"><label class="form-label">Reason</label><input type="text" name="reason" class="form-control" value="Stock transfer"></div>
...
function checkStock(){
    var p=document.getElementById('product_id').value,w=document.getElementById('from_warehouse_id').value,r=document.getElementById('from_rack_id').value,l=document.getElementById('lot_id').value,v=document.getElementById('variation_id').value;
