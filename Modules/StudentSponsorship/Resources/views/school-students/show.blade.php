...
<div style="padding: 20px; max-width: 1200px; margin: 0 auto;">
    ...
    <div class="action-buttons">
        <a href="{{ route('admin.studentsponsorship.school-students.edit', $student->hash_id) }}" class="btn btn-primary">
            Edit
        </a>
    ...
    <div class="profile-header">
        <div class="profile-header-content">
            ...
            <div class="profile-info">
                ...
                <div class="profile-id">{{ $student->school_internal_id }} • School ID: {{ $student->school_student_id }}</div>
                <div class="profile-badges">
                    <span class="profile-badge {{ $student->status ? 'active' : 'inactive' }}">
        ...
        <div class="card-body">
            <div class="info-grid">
                ...
                <div class="info-item">
                    ...
                    <span class="info-value">{{ $student->postal_code ?: 'Not provided' }}</span>
                </div>
                <div class="info-item">
        ...
        <div class="card-body">
            <div class="info-grid">
                <div class="info-item">
                    ...
                    <span class="info-value">{{ $student->dob?->format('M d, Y') ?: 'Not provided' }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Age</span>
                    <span class="info-value">{{ $student->age ?: 'Not provided' }}</span>
                </div>
                <div class="info-item">
    ...
    <div class="card info-card">
        <div class="card-header">
            <i class="fas fa-graduation-cap me-2"></i>School Information
        </div>
        <div class="card-body">
            <div class="info-grid">
                <div class="info-item">
                    <span class="info-label">School Name</span>
                    <span class="info-value">{{ $student->school_name_display ?: 'Not provided' }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">School Type</span>
                    <span class="info-value">{{ $student->school_type ?: 'Not provided' }}</span>
                </div>
                <div class="info-item">
                ...
                <div class="info-item">
                    <span class="info-label">Grade Mismatch Reason</span>
                    <span class="info-value" style="color: #B45309;">{{ $student->grade_mismatch_reason }}</span>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="card-body">
            <div class="info-grid">
                <div class="info-item">
                    ...
                    <span class="info-value">{{ $student->sponsorship_start_date?->format('M d, Y') ?: 'Not provided' }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Sponsorship End</span>
                    <span class="info-value">{{ $student->sponsorship_end_date?->format('M d, Y') ?: 'Not provided' }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Introduced By</span>
                    <span class="info-value">{{ $student->introduced_by ?: 'Not provided' }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Introducer Phone</span>
                    <span class="info-value">{{ $student->introducer_phone ?: 'Not provided' }}</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body">
            <div class="info-grid">
                <div class="info-item">
                    ...
                    <span class="info-value">{{ $student->bank_name_display ?: 'Not provided' }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Account Number</span>
                    <span class="info-value">{{ $student->bank_account_number ?: 'Not provided' }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Branch Info</span>
                    <span class="info-value">{{ $student->bank_branch_info ?: 'Not provided' }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Branch Number</span>
                    <span class="info-value">{{ $student->bank_branch_number ?: 'Not provided' }}</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body">
            <div class="info-grid">
                <div class="info-item">
                    ...
                    <span class="info-value">{{ $student->father_name ?: 'Not provided' }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Father's Income</span>
                    <span class="info-value">{{ $student->father_income ? number_format($student->father_income, 2) : 'Not provided' }}</span>
                </div>
                <div class="info-item">
                ...
                <div class="info-item">
                    ...
                    <span class="info-value">{{ $student->mother_income ? number_format($student->mother_income, 2) : 'Not provided' }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Guardian's Name</span>
                    <span class="info-value">{{ $student->guardian_name ?: 'Not provided' }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Guardian's Income</span>
                    <span class="info-value">{{ $student->guardian_income ? number_format($student->guardian_income, 2) : 'Not provided' }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card info-card">
        <div class="card-header">
            ...
            Report Cards ({{ $reportCards->count() }})
        </div>
        <div class="card-body">
            @if($reportCards->count() > 0)
                <div class="report-cards-list" style="display: flex; flex-direction: column; gap: 12px; margin-bottom: 20px;">
                    @foreach($reportCards as $rc)
                        <div class="report-card-row" id="report-card-{{ $rc->id }}" style="display: flex; align-items: center; justify-content: space-between; padding: 12px 16px; background: var(--body-bg); border: 1px solid var(--card-border); border-radius: 8px;">
                            <div style="display: flex; align-items: center; gap: 12px;">
                                ...
                                <div>
                                    <div style="font-weight: 600; font-size: 14px; display: flex; align-items: center; gap: 8px;">
                                        {{ $rc->filename }}
                                        @php $status = $rc->status ?? 'pending'; @endphp
                                        @if($status === 'pending')
                                    ...
                                    <div style="font-size: 12px; color: var(--text-muted);">{{ $rc->term_display }} • {{ \Carbon\Carbon::parse($rc->upload_date)->format('M d, Y') }}</div>
                                </div>
                            </div>
                            <div style="display: flex; gap: 8px; align-items: center;">
                                @if($status === 'pending')
                                    <button type="button" onclick="approveReportCard({{ $rc->id }})" 
                                    ...
                                    <button type="button" onclick="rejectReportCard({{ $rc->id }})" 
                                ...
                                @endif
                                <a href="{{ route('admin.studentsponsorship.school-students.download-report-card', [$student->hash_id, $rc->id]) }}" 
                                ...
                                <button type="button" onclick="deleteReportCard({{ $rc->id }})" 
            ...
            <div style="padding-top: 16px; border-top: 1px solid var(--card-border);">
                ...
                <form id="report-card-upload-form" enctype="multipart/form-data">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 12px; align-items: end;">
                        ...
                        <div>
                            <label style="display: block; font-size: 12px; font-weight: 600; margin-bottom: 6px; color: var(--text-muted);">UPLOAD DATE <span style="color: red;">*</span></label>
                            <input type="date" id="rc-upload-date" required style="padding: 10px; border: 1px solid var(--card-border); border-radius: 6px; width: 100%;" value="{{ date('Y-m-d') }}">
                        </div>
                        <div>
                            <label style="display: block; font-size: 12px; font-weight: 600; margin-bottom: 6px; color: var(--text-muted);">FILE (PDF) <span style="color: red;">*</span></label>
...
function loadPortalStatus() {
    fetch('{{ route("admin.studentsponsorship.school-students.portal-status", $student->hash_id) }}')
...
function checkEmail() {
    ...
    fetch('{{ route("admin.studentsponsorship.school-students.check-email") }}', {
        method: 'POST',
        headers: {
...
function createPortalAccount() {
    ...
    fetch('{{ route("admin.studentsponsorship.school-students.create-portal", $student->hash_id) }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function togglePortalStatus(activate) {
    const url = activate 
        ? '{{ route("admin.studentsponsorship.school-students.activate-portal", $student->hash_id) }}'
        : '{{ route("admin.studentsponsorship.school-students.deactivate-portal", $student->hash_id) }}';
...
function resetPassword() {
    ...
    fetch('{{ route("admin.studentsponsorship.school-students.reset-portal-password", $student->hash_id) }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
document.getElementById('report-card-upload-form').addEventListener('submit', function(e) {
    ...
    const uploadDate = document.getElementById('rc-upload-date').value;
...
function deleteReportCard(id) {
    ...
    fetch('{{ url("admin/studentsponsorship/school-students/" . $student->hash_id . "/report-cards") }}/' + id, {
        method: 'DELETE',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function approveReportCard(id) {
    ...
    fetch('{{ url("admin/studentsponsorship/school-students/" . $student->hash_id . "/report-cards") }}/' + id + '/approve', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function submitRejection() {
    ...
    fetch('{{ url("admin/studentsponsorship/school-students/" . $student->hash_id . "/report-cards") }}/' + rejectingReportCardId + '/reject', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
