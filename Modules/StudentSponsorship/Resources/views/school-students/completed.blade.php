<style>
    ...
    .grade-badge { display: inline-flex; padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; background: #dcfce7; color: #166534; }
...
<div style="padding: 20px;">
    <div class="page-header">
        <h1><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>Completed School Students</h1>
    </div>
    ...
    <div class="rollback-section">
        <div class="rollback-info">
            ...
            <div class="rollback-text">
                ...
                <p>Process all in-progress students: mark as completed, promote grade, remove portal access, set inactive.</p>
            </div>
        </div>
    ...
    <div class="table-card">
        <div class="table-card-header">
            ...
            <div class="filter-section">
                ...
                <select class="filter-select" id="gradeFilter" data-dt-filter="grade" data-dt-table="completedStudentsTable">
                    ...
                    @foreach($grades as $key => $label)<option value="{{ $key }}">{{ $label }}</option>@endforeach
                </select>
                @if($schools->count() > 0)
        ...
        <div class="table-card-body">
            <table id="completedStudentsTable" class="dt-table dt-search dt-perpage" data-route="{{ route('admin.studentsponsorship.school-students.completed.data') }}">
                <thead><tr>
                    ...
                    <th class="dt-sort" data-col="completed_year" data-render="year_cell">Completed Year</th>
                    <th data-col="school_name">School</th>
                    <th data-col="status" data-render="status_badge">Status</th>
    ...
    <div class="modal-box">
        ...
        <div class="modal-body">
            <ul class="confirm-list">
                ...
                <li><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 10.5L12 3m0 0l7.5 7.5M12 3v18" /></svg>Promote grade by <strong>+1</strong></li>
        ...
        <div class="modal-footer">
            ...
            <button class="modal-btn primary" onclick="showGradeSelect()">Next: Select Grade</button>
        </div>
    </div>
</div>

<!-- Grade Selection Modal -->
<div class="modal-overlay" id="gradeSelectModal">
    <div class="modal-box">
        <div class="modal-header">
        ...
        <div class="modal-footer">
            <button class="modal-btn secondary" onclick="hideModal('gradeSelectModal');showModal('confirmModal')">Back</button>
    ...
    <div class="modal-box">
        ...
        <div class="modal-body">
            ...
            <ul class="result-details">
                ...
                <li><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg>Grades promoted by +1</li>
...
<script>
var defaultAvatar = "data:image/svg+xml;base64,";
...
window.dtRenders.student_cell = function(value, row) {
    ...
    return '<div class="student-cell"><img src="' + avatar + '" alt="" class="student-avatar" onerror="this.src=\'' + defaultAvatar + '\'"><div class="student-info"><div class="student-name">' + nameHtml + '</div><div class="student-id">ID: ' + (row.school_internal_id || '-') + '</div></div></div>';
};

window.dtRenders.grade_badge = function(value) { return value ? '<span class="grade-badge">Grade ' + value + '</span>' : '-'; };
window.dtRenders.year_cell = function(value) { return value ? '<span style="background:#E0E7FF;color:#3730A3;padding:4px 10px;border-radius:12px;font-size:11px;font-weight:600;">' + value + '</span>' : '<span style="color:#9CA3AF;">-</span>'; };
...
function showGradeSelect() { hideModal('confirmModal'); showModal('gradeSelectModal'); }
...
function startRollback() {
    ...
    setTimeout(function() {
        ...
        fetch('{{ route("admin.studentsponsorship.school-students.rollback-all") }}', {
            method: 'POST',
        ...
        .then(function(data) {
            if (data.success) {
                ...
                setTimeout(function() {
                    setStep(3, 'active', 'Updating grades...');
                    document.getElementById('progressBar').style.width = '70%';
                    
                    setTimeout(function() {
                        setStep(3, 'done', 'Grades promoted');
