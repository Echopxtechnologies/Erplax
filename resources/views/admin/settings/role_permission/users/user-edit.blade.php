{{-- <x-layouts.app> --}}
    <style>
        ...
        .page-header .badges { display: flex; gap: 8px; margin-top: 8px; }
        .page-header .badge { padding: 4px 10px; background: #f3f4f6; border-radius: 6px; font-size: 12px; color: #6b7280; display: flex; align-items: center; gap: 5px; }
        ...
        .info-box {
            ...
            background: #eff6ff; border: 1px solid #bfdbfe; border-radius: 10px; margin-bottom: 20px;
        }
        .info-box i { color: #3b82f6; }
        .info-box span { font-size: 13px; color: #1e40af; }
        
        .status-box {
            display: flex; align-items: center; gap: 14px; padding: 16px 20px;
            background: #f9fafb; border-radius: 12px; border: 1px solid #e5e7eb;
        }
        .status-box.disabled { opacity: 0.6; cursor: not-allowed; }
    ...
    @php
        $firstName = old('first_name', $staff->first_name ?? '');
        $lastName = old('last_name', $staff->last_name ?? '');
    ...
    <div class="staff-page">
        <div class="page-header">
            <div class="title">
                ...
                <div class="badges">
                    @if($staff && $staff->employee_code)
                        <span class="badge"><i class="fas fa-id-badge"></i> {{ $staff->employee_code }}</span>
                    @endif
                    @if($staff && $staff->join_date)
                        <span class="badge"><i class="fas fa-calendar"></i> Joined {{ $staff->join_date->format('M d, Y') }}</span>
                    @endif
                </div>
            </div>
        ...
        <form action="{{ route('admin.settings.users.update', $user->id) }}" method="POST" id="staffForm">
            @csrf
            ...
            <div class="section-card">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" name="first_name" value="{{ $firstName }}" placeholder="Enter first name" required>
                    </div>
                    <div class="form-group">
                        <label>Last Name <span class="req">*</span></label>
                        <input type="text" name="last_name" value="{{ $lastName }}" placeholder="Enter last name" required>
                    </div>
                    <div class="form-group">
                        <label>Email <span class="req">*</span></label>
                        <input type="email" name="email" value="{{ old('email', $user->email) }}" placeholder="email@example.com" required>
                    </div>
                    <div class="form-group">
                        <label>Phone</label>
                        <input type="text" name="phone" value="{{ old('phone', $staff->phone) }}" placeholder="+91 98765 43210">
                    </div>
                    <div class="form-group">
                        <label>Date of Birth</label>
                        <input type="date" name="dob" value="{{ old('dob', $staff->dob?->format('Y-m-d')) }}">
                    </div>
                    <div class="form-group">
                        <label>Gender</label>
                        <select name="gender">
            ...
            <div class="section-card">
                ...
                <div class="form-grid cols-4">
                    <div class="form-group">
                        ...
                        <input type="text" name="employee_code" value="{{ old('employee_code', $staff->employee_code) }}" placeholder="Employee code">
                    </div>
                    <div class="form-group">
                        <label>Department</label>
                        <input type="text" name="department" value="{{ old('department', $staff->department) }}" placeholder="e.g. Engineering">
                    </div>
                    <div class="form-group">
                        <label>Designation</label>
                        <input type="text" name="designation" value="{{ old('designation', $staff->designation) }}" placeholder="e.g. Manager">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="confirmation_date" value="{{ old('confirmation_date', $staff->confirmation_date?->format('Y-m-d')) }}">
                    </div>
                    <div class="form-group">
                        <label>Exit Date</label>
                        <input type="date" name="exit_date" value="{{ old('exit_date', $staff->exit_date?->format('Y-m-d')) }}">
                    </div>
                </div>
            </div>
            ...
            <div class="section-card">
                ...
                <div class="form-grid">
                    <div class="form-group span-2">
                        ...
                        <input type="text" name="address_line1" value="{{ old('address_line1', $staff->address_line1) }}" placeholder="Street address">
                    </div>
                    <div class="form-group">
                        <label>Address Line 2</label>
                        <input type="text" name="address_line2" value="{{ old('address_line2', $staff->address_line2) }}" placeholder="Apt, suite, building">
                    </div>
                    <div class="form-group">
                        <label>City</label>
                        <input type="text" name="city" value="{{ old('city', $staff->city) }}" placeholder="City">
                    </div>
                    <div class="form-group">
                        <label>State</label>
                        <input type="text" name="state" value="{{ old('state', $staff->state) }}" placeholder="State">
                    </div>
                    <div class="form-group">
                        <label>Pincode</label>
                        <input type="text" name="pincode" value="{{ old('pincode', $staff->pincode) }}" placeholder="Pincode">
                    </div>
                    <div class="form-group">
                        <label>Country</label>
                        <input type="text" name="country" value="{{ old('country', $staff->country ?? 'India') }}" placeholder="Country">
                    </div>
                </div>
            </div>
            ...
            <div class="section-card">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" name="emergency_contact_name" value="{{ old('emergency_contact_name', $staff->emergency_contact_name) }}" placeholder="Full name">
                    </div>
                    <div class="form-group">
                        <label>Contact Phone</label>
                        <input type="text" name="emergency_contact_phone" value="{{ old('emergency_contact_phone', $staff->emergency_contact_phone) }}" placeholder="Phone number">
                    </div>
                    <div class="form-group">
                        ...
                        <select name="emergency_contact_relation">
                            ...
                            @foreach(['Father', 'Mother', 'Spouse', 'Sibling', 'Friend', 'Other'] as $rel)
                                <option value="{{ $rel }}" {{ old('emergency_contact_relation', $staff->emergency_contact_relation) === $rel ? 'selected' : '' }}>{{ $rel }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="section-card">
                ...
                @if($isSelf)
                    <label class="status-box disabled" style="margin-bottom: 24px;">
                        <div class="toggle"><input type="checkbox" checked disabled><span class="toggle-track"></span></div>
                        <div class="status-text"><strong>Account Active</strong><span>You cannot deactivate your own account</span></div>
                    </label>
                    ...
                    @if($roles->count() > 0)
                        <div class="roles-grid">
                            @foreach($roles as $role)
                                <label class="role-chip {{ in_array($role->name, old('roles', $userRoles ?? [])) ? 'selected' : '' }}">
    ...
    <script>
        ...
        document.getElementById('staffForm').addEventListener('submit', function(e) {
            ...
            if (pw && pw !== pc) { alert('Passwords do not match'); e.preventDefault(); return; }
