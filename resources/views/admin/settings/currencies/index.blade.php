    ...
    <div class="table-card">
        ...
        <div class="table-card-body">
            <table class="dt-table dt-search dt-perpage" 
                   id="currenciesTable"
                   data-route="{{ route('admin.settings.currencies.data') }}">
                <thead>
                    <tr>
                        <th class="dt-sort" data-col="code">Code</th>
                        <th class="dt-sort" data-col="name">Name</th>
                        <th data-col="symbol" data-render="symbol">Symbol</th>
                        <th class="dt-sort" data-col="exchange_rate" data-render="exchange_rate">Exchange Rate</th>
    ...
    <div class="modal-content">
        <div class="modal-header">
            <h3>
                ...
                <span id="modalTitle">Add Currency</span>
            </h3>
        ...
        <form id="currencyForm">
            <div class="modal-body">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="code" id="code" class="form-control" maxlength="3" placeholder="INR" required style="text-transform: uppercase;">
                        <div class="form-error" id="codeError"></div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Symbol <span class="required">*</span></label>
                        <input type="text" name="symbol" id="symbol" class="form-control" maxlength="10" placeholder="₹" required>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" id="name" class="form-control" placeholder="Indian Rupee" required>
                    <div class="form-error" id="nameError"></div>
                </div>

                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label">Exchange Rate <span class="required">*</span></label>
                        <input type="number" name="exchange_rate" id="exchange_rate" class="form-control" step="0.0001" min="0" placeholder="1.0000" required>
                        <div class="form-error" id="exchange_rateError"></div>
                    </div>
                    <div class="form-group">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary">
                    ...
                    <span id="submitBtnText">Save Currency</span>
                </button>
            </div>
        </form>
    </div>
...
<div class="modal-overlay" id="deleteModal">
    <div class="modal-content" style="max-width: 400px;">
        ...
        <div class="modal-body">
            <p style="margin: 0; color: var(--text-primary);">Are you sure you want to delete <strong id="deleteCurrencyName"></strong>?</p>
...
window.dtRenders.symbol = function(data, row) {
    return '<span class="currency-symbol">' + row.symbol + '</span>';
};

window.dtRenders.exchange_rate = function(data, row) {
    return '<span class="exchange-rate">' + row.exchange_rate_display + '</span>';
};
...
window.dtRenders.actions = function(data, row) {
    return `
        <div style="display: flex; gap: 8px;">
            <button type="button" class="btn" style="padding: 6px 12px; font-size: 12px;" onclick='editCurrency(${JSON.stringify(row)})'>
...
function openModal() {
    ...
    document.getElementById('submitBtnText').textContent = 'Save Currency';
...
function editCurrency(row) {
    ...
    document.getElementById('submitBtnText').textContent = 'Update Currency';
    ...
    document.getElementById('exchange_rate').value = row.exchange_rate || 1;
    document.getElementById('decimal_places').value = row.decimal_places || 2;
    document.getElementById('symbol_position').value = row.symbol_position || 'before';
    document.getElementById('thousand_separator').value = row.thousand_separator || ',';
...
document.getElementById('currencyForm').addEventListener('submit', async function(e) {
    ...
    const url = id 
        ? '{{ route("admin.settings.currencies.update", ":id") }}'.replace(':id', id)
    ...
    try {
        ...
        if (response.ok && result.success) {
            closeModal();
            if (window.dtRefresh) {
                window.dtRefresh('currenciesTable');
            } else {
                location.reload();
            }
...
document.getElementById('confirmDeleteBtn').addEventListener('click', async function() {
    ...
    const url = '{{ route("admin.settings.currencies.destroy", ":id") }}'.replace(':id', deleteId);
    ...
    try {
        ...
        if (response.ok && result.success) {
            ...
            alert(result.message || 'Cannot delete this currency');
        }
    } catch (error) {
        console.error('Error:', error);
