...
<style>
    ...
    .btn { display: inline-flex; align-items: center; justify-content: center; gap: 8px; padding: 10px 18px; border-radius: 8px; font-weight: 600; font-size: 14px; cursor: pointer; transition: all 0.2s; text-decoration: none; border: none; }
    ...
    /* Tax Badges in table - Odoo style */
    ...
    @media print { .header-actions, .btn, .modal-backdrop { display: none !important; } .two-column { grid-template-columns: 1fr; } }
</style>
@endpush

@php
    // Load taxes for lookup
    ...
    // Calculate tax breakdown from items using multiple tax_ids
    $taxBreakdown = [];
    $totalTax = 0;
    ...
    // Fallback to invoice tax if no per-item taxes
...
<div class="page-wrapper">
    <div class="page-header">
        <h1 class="page-title"><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>Invoice {{ $invoice->invoice_number }}</h1>
        <div class="header-actions">
            <a href="{{ route('admin.sales.invoices.index') }}" class="btn btn-secondary"><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>Back</a>
            <a href="{{ route('admin.sales.invoices.edit', $invoice->id) }}" class="btn btn-secondary"><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>Edit</a>
            <button onclick="window.print()" class="btn btn-secondary"><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>Print</button>
            @if($invoice->amount_due > 0)<button onclick="openPaymentModal()" class="btn btn-success"><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg>Record Payment</button>@endif
        </div>
    </div>
    ...
    <div class="two-column">
        <div class="left-column">
            <div class="card">
                <div class="card-header"><h3><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>Bill To</h3><span class="status-badge status-{{ $invoice->status }}">{{ ucfirst($invoice->status) }}</span></div>
            ...
            <div class="card">
                <div class="card-header"><h3><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>Invoice Items</h3></div>
                                                ...
                                                <div class="tax-badges">
                                                    @foreach($itemTaxIds as $taxId)
                                                        @php
                                                            ...
                                                            $taxName = $taxNamesMap[$taxId] ?? 'Tax';
                                                        @endphp
                                                        <span class="tax-badge">{{ $taxRate }}% {{ $taxName }}</span>
                                                    @endforeach
                                                </div>
                                            @else
                ...
                <div class="totals-section">
                    ...
                    <div class="tax-breakdown-section">
                        <div class="tax-breakdown-title"><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="width:14px;height:14px;"><path stroke-linecap="round" stroke-linejoin="round" d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z"/></svg>Tax Summary</div>
            ...
            <div class="card">
                <div class="card-header"><h3><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>Notes & Terms</h3></div>
                <div class="card-body">
        ...
        <div class="right-column">
            ...
            <div class="card">
                <div class="card-header"><h3><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>Invoice Details</h3></div>
                <div class="card-body">
                    <div class="info-grid">
                        ...
                        @if($invoice->assigned_to)<div class="info-row"><span class="info-label">Assigned To</span><span class="info-value">{{ $invoice->assigned_to }}</span></div>@endif
                    </div>
                </div>
            </div>

            <div class="card">
                <div class="card-header"><h3><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/></svg>Actions</h3></div>
                <div class="card-body" style="display: flex; flex-direction: column; gap: 10px;">
                    ...
                    @if($invoice->status === 'cancelled')<button type="button" class="btn btn-warning" onclick="updateStatus('draft')" style="width: 100%;">Revert to Draft</button>@endif
                    @if($invoice->payment_status === 'paid')<div class="status-message success"><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="width:20px;height:20px;"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>Invoice Paid in Full</div>@endif
                </div>
            </div>

            <div class="card">
                <div class="card-header"><h3><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg>Payment History</h3></div>
                <div class="card-body">
                    @if($invoice->payments && $invoice->payments->count() > 0)
                        @foreach($invoice->payments as $payment)<div class="payment-item"><div class="payment-info"><div class="payment-number">{{ $payment->payment_number }}</div><div class="payment-meta">{{ $payment->payment_date->format('d M Y') }} • {{ ucfirst(str_replace('_', ' ', $payment->payment_method)) }}</div></div><div class="payment-amount">{{ $invoice->currency ?? 'INR' }} {{ number_format($payment->amount, 2) }}</div></div>@endforeach
                    @else<p class="empty-state">No payments recorded yet</p>@endif
                </div>
            </div>

            <div class="card">
                <div class="card-header"><h3><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>Activity</h3></div>
                <div class="card-body">
                    <div class="timeline">
                        ...
                        @if($invoice->payments && $invoice->payments->count() > 0)@foreach($invoice->payments as $payment)<div class="timeline-item"><div class="timeline-dot success"></div><div class="timeline-content"><div class="title">Payment - {{ $invoice->currency ?? 'INR' }} {{ number_format($payment->amount, 2) }}</div><div class="meta">{{ $payment->created_at->format('d M Y, h:i A') }}</div></div></div>@endforeach@endif
...
function submitPayment(e) {
    ...
    fetch('{{ route("admin.sales.invoices.recordPayment", $invoice->id) }}', { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' }, body: formData })
    .then(res => res.json()).then(data => { if (data.success) { alert(data.message); window.location.reload(); } else { alert(data.message || 'Error'); } }).catch(err => alert('Error recording payment'));
}
function updateStatus(status) {
    if (!confirm(`Change status to "${status}"?`)) return;
    fetch('{{ route("admin.sales.invoices.updateStatus", $invoice->id) }}', { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Content-Type': 'application/json', 'Accept': 'application/json' }, body: JSON.stringify({ status: status }) })
    .then(r => r.json()).then(data => { if (data.success) { alert(data.message || 'Updated'); location.reload(); } else { alert(data.message || 'Error'); } }).catch(() => alert('Error'));
}
