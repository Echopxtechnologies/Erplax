...
<head>
    ...
    <title>{{ isset($invoice) ? 'Edit Invoice' : 'New Invoice' }}</title>
    <style>
        /* Reset & Base */
        *, *::before, *::after {
...
<form action="{{ isset($invoice) ? route('admin.sales.invoices.update', $invoice->id) : route('admin.sales.invoices.store') }}" method="POST" id="invoiceForm">
@csrf
...
<div class="invoice-form-wrapper">
    ...
    <div class="invoice-header">
        <h1 class="invoice-title">{{ isset($invoice) ? 'Edit Invoice' : 'New Invoice' }}</h1>
        <div class="header-buttons">
            <a href="{{ route('admin.sales.invoices.index') }}" class="btn btn-outline">
                <span>✕</span> Discard
            </a>
        ...
        <div class="card-body">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="date" name="date" class="form-control" value="{{ isset($invoice) ? $invoice->date->format('Y-m-d') : date('Y-m-d') }}" required>
                </div>
                <div class="form-group">
                    <label class="form-label">Due Date</label>
                    <input type="date" name="due_date" class="form-control" value="{{ isset($invoice) && $invoice->due_date ? $invoice->due_date->format('Y-m-d') : date('Y-m-d', strtotime('+30 days')) }}">
                </div>
                <div class="form-group">
                    <label class="form-label">Currency</label>
                    <select name="currency" class="form-control">
                        <option value="INR" {{ (isset($invoice) && $invoice->currency == 'INR') ? 'selected' : '' }}>₹ INR</option>
                        <option value="USD" {{ (isset($invoice) && $invoice->currency == 'USD') ? 'selected' : '' }}>$ USD</option>
                        <option value="EUR" {{ (isset($invoice) && $invoice->currency == 'EUR') ? 'selected' : '' }}>€ EUR</option>
                    </select>
                </div>
                <div class="form-group">
            ...
            <div class="totals-wrapper">
                <div class="totals-box">
                    ...
                    <div class="totals-row">
                        ...
                        <input type="number" name="discount_percent" id="discountPercent" class="totals-input" value="{{ $invoice->discount_percent ?? 0 }}" step="0.01" min="0" max="100">
                    </div>
        ...
        <div id="tab-other" class="tab-content">
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="email" name="email" id="email" class="form-control" value="{{ $invoice->email ?? '' }}" placeholder="customer@email.com">
                </div>
                <div class="form-group">
                    <label class="form-label">Phone</label>
                    <input type="text" name="phone" id="phone" class="form-control" value="{{ $invoice->phone ?? '' }}" placeholder="+91 XXXXX XXXXX">
                </div>
                <div class="form-group col-span-2">
                    <label class="form-label">Address</label>
                    <textarea name="address" id="address" class="form-control" rows="2" placeholder="Street address">{{ $invoice->address ?? '' }}</textarea>
                </div>
            </div>
            <div class="form-row">
                <div class="form-group">
                    <label class="form-label">City</label>
                    <input type="text" name="city" id="city" class="form-control" value="{{ $invoice->city ?? '' }}" placeholder="City">
                </div>
                <div class="form-group">
                    <label class="form-label">State</label>
                    <input type="text" name="state" id="state" class="form-control" value="{{ $invoice->state ?? '' }}" placeholder="State">
                </div>
                <div class="form-group">
                    <label class="form-label">Zip Code</label>
                    <input type="text" name="zip_code" id="zip_code" class="form-control" value="{{ $invoice->zip_code ?? '' }}" placeholder="000000">
                </div>
                <div class="form-group">
                    <label class="form-label">Country</label>
                    <input type="text" name="country" id="country" class="form-control" value="{{ $invoice->country ?? '' }}" placeholder="Country">
                </div>
            </div>
            <div class="form-row">
                <div class="form-group col-span-2">
                    <label class="form-label">Customer Notes</label>
                    <textarea name="content" class="form-control" rows="3" placeholder="Notes visible to customer">{{ $invoice->content ?? '' }}</textarea>
                </div>
                <div class="form-group col-span-2">
                    <label class="form-label">Internal Notes</label>
                    <textarea name="admin_note" class="form-control" rows="3" placeholder="Private notes (not visible to customer)">{{ $invoice->admin_note ?? '' }}</textarea>
                </div>
            </div>
            <div class="form-row">
                <div class="form-group" style="grid-column: span 4;">
                    <label class="form-label">Terms & Conditions</label>
                    <textarea name="terms_conditions" class="form-control" rows="4" placeholder="Terms and conditions for this invoice">{{ $invoice->terms_conditions ?? '' }}</textarea>
                </div>
            </div>
        </div>
    </div>
...
const existingItems = @json(isset($invoice) && $invoice->items ? $invoice->items->toArray() : []);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('customer_id').addEventListener('change', function() {
        const id = this.value;
        if (id) {
            fetch(`{{ url('admin/sales/invoices/customer') }}/${id}`)
    ...
    // Form validation
    document.getElementById('invoiceForm').addEventListener('submit', validateForm);
});
...
function showProducts(el) {
    ...
    fetch('{{ route("admin.sales.invoices.searchProducts") }}?q=')
        .then(res => res.json())
        .then(renderProducts);
    dropdown.classList.add('show');
...
function filterProducts() {
    ...
    fetch(`{{ route("admin.sales.invoices.searchProducts") }}?q=${encodeURIComponent(search)}`)
        .then(res => res.json())
        .then(renderProducts);
}
...
function showError(message) {
    ...
    invoiceHeader.insertAdjacentElement('afterend', errorBox);
}
